...
public:
	...
	// edge_spacing_x and edge_spacing_y the spaving of the image within the search region to account for edge effects.
	...
	// Get the size bounding box center x and y coordinates.
	double get_cneter_x() const;
	double get_center_y() const;
	...
	// This might be 0, but might be > 0 if the output is near the edge of the image.
	double edge_spacing_x() const;
	double edge_spacing_y() const;
	
	// Aree of intersection between two bounding boxes
	double compute_area() const;
	
	// Area of intersection between two bounding boxes.
	double compute_intersection(const BoundingBox& bbox) const;
	
	// Bounding box coordinate: top left, bottom right
	double x1_, y1_, x2_, y2_;
	
	// Factor to sacle the bounding box coordinates before inputting into the neural net.
