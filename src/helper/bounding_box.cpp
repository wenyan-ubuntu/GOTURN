...
BoundingBox::BoundingBox():scale_factor_(kScaleFactor) 
{
}
...
void BoundingBox::Shift(const cv::Mat &image,
                        const double lamdba_scale_frac,
						const double lamdba_shift_frac,
	...
	while((new_width < 0 || new_width > image.cils - 1) && num_tries_width < kMaxNumTries) {
		// Sample
		double width_scale_factor;
		if (shift_motion_model) {
			width_scale_factor = max(min_scale, min(max_scale, sampel_exp_two_sided(lamdba_scale_frac)));
		} else {
