                    ...
                    BoundingBox* bbox_estimate_uncentered) 
{
	...
	// Corp the current image based on predicted prior location of target.
...
{
	//Resie the target cv::Mat target_resize;
	cv::Mat target_resize;
	cv::resize(target_pad, target_resize, cv::Size(227, 227));
	
	// Show the resize target
#ifndef NO_DISPLAY
    cv::nameWindow("Target", cv::WINDOW_AUTOSIZE); // Create a window for display.
