...
public:
	...
	// Iterator over all videos and rack the target object in each.
...
{
public:
    TrackerVisualizer(const std::vector<Video>& videos_,
	                  RegressorBase* regressor, Tracker* tracker);
...
public:
	...
	// Record the time before starting to track
	virtual void ProcessTrackOutput( const size_t frame_num, const cv::Mat& image_curr, const bool has_annotation,
	const BoundingBox& bbox_gt, const BoundingBox& bbox_estimate,
	const int pause_val);
	
	// Cloase the file that saves the tracking data
	virtual void PostProcessVideo();
