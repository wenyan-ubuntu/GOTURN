...
void TrackerVisualizer::ProcessTrackOutput(const size_t frame_num, 
                        const cv::Mat& image_curr, 
						const bool has_annotation,
                        const BoundingBox& bbox_gt, 
						const BoundingBox& bbox_estimate_uncentered,
                        const int pause_val)
{
	cv::Mat fulloutput;
	image_curr.copy(full_output);
    ...
    const int pause_val)
{
    ...
    hrt_.stop();
    const double ms = hrt_getMilliseconds();
