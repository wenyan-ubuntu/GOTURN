...
// If true, only load a small number of imagesize
		...
		:path_(image_folder)
{
	...
	find_subfolders(annotations_folder, &subfolders); // helper/helper
	...
	for (size_t i = 0; i < max_subfolders; ++i) {
		...
		// printg("Loading subfolder: %s\n", subfolder_name.c_str());
		
		// Find te annotation files 
		...
		for (size_t j = 0; j < annotation_files.size(); ++j) {
			...
			// printf("Processing annotation file: %\n". full_path.c_str());
                                          ...
                                          vector<Annotation>* image_annotations)
{
	...
	const int display_height = atoi(size->FirstChildElemetn("height")->GetText());
...
#ifndef NO_DISPLAY
        // Display the image and wait for a keystorke to continue
		// Create a window for display
	    cv::nameWindow("Display window", cv::WINDOW_AUTOSIZE);
		// Show our image inside it
		cv::imshow("Display window", image);
		// Wait for a keystorke in the window
		cv::waitKey(0);
#endif
	}
}

void LoaderImagenetDet::ComputeStatics() const 
{
...
void LoaderImagenetDet::LoadImage(const size_t image_num, cv::Mat* image) const 
{
...
void LoaderImagenetDet::ShowAnnotationsRand() const 
{
	while (true) {
		...
		// Get the anotations for this image
		const std::vector<Annotation>& annotations = images_[image_num];
...
void LoaderImagenetDet::ShowAnnotationsShift() const 
{
	...
	for (size_t i = 0; i < images_.size(); i++) {
		...
		for (size_t j = 0; j < annotations.size(); j++) {
			...
			// show the full image with the bounding bx annotation
			cv::Mat image_copy;
			image.copyTo(image_copy);
