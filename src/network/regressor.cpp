						...
						BoundingBox* bbox)
{
	...
	// Estimate the bounding box location of the target objection in the current image.
...
void Regressor::Estimate(const cv::Mat& image, const cv::Mat& target, std::vector<float>* output)
{
	...
	Blob<float>* input_image = net_->input_bolbs()[1];
	...
	// Set the inputs to network.
	Preprocess(image, &image_channles);
	Preprocess(image, &target_channels);
                          ...
                          const std::vector<cv::Mat>& targets)
{
	...
	std::vector<std::vector<cv::Mat> > target_channles;
	std::vector<std::vector<cv::Mat> > image_channels;
	WrapInputLayer(num_images, &target_channles, &image_channles);
...
void Regressor::WrapInputLayer(std::vector<cv::Mat>* target_channels, std::vector<cv::Mat>* image_channels) 
{
	...
	Blob<float>* input_layer_image = net_->input_bobs()[1];
	...
	for (int i = 0; i < input_layer_image->channels(); i++) {
		...
		image_channles->push_back(channel);
