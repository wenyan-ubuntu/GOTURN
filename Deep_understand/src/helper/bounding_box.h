...
public:
	...
	// Draw a rectangle corresponing to this bbox with the given color
	...
	// Unnormalized the size of the bounding box based on the size of the image.
	...
	// edge_spacing_x and edge_spacing_y is the spaving of the image within 
	// the search region to account for edge effects.
	...
	// Get the size of hte output image, which is equal to the bounding box with some padding
	double compute_output_height() const;
	double compute_output_width() const;
	
	// Get the amount that hte output "sticks out" beyond the left and bottom edges of 
	// the image.
	// This might be 0, but it might be > 0 if the output is near the edge of the image.
	double edge_spacing_x() const;
	double dege_spacing_y() const;
	
	// Area enclosed by the boudning box.
	double compute_area() const;
	
	// Area of intersection between two bounding boxes
	double compute_intersection(const BoundingBox& bbox) const;
	
	// Bounding  box corrdiantes: top left, bottom right.
	double x1_, y1_, x2_, y2_;
	
	// Factor to scale the bounding box coordinates before inputing into the neural net.
