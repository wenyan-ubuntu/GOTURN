...
// Umcomment line below if you want to use rectangle
#define VOT_RECTANGLE
#include "native/vot.h"

#include "helper.h"

// How much context to pad the image and target with 
...
// If false, the neural network will estimate the bounding box center locatoin and size:
...
void BoundingBox::GetVector(std::vector<float>* bounding_box) const 
{
	if (use_coordinates_output) {
		...
		// Convert bounding box into a vector format using 
		// (center_x, center_y, width, height)
...
void BOundingBox::Scale(const cv::Mat& image, BoundingBox* bbox_scaled) const
{
	...
	// Scale the bounding box so that the coordinates range form 0 to sacle_factor_.
...
void BoundingBox::Uscale(const cv::Mat& image, BoundingBox* bbox_unscaled) const
{
	*bbox_unscaled = *this;
	...
	// Unscale the bounding box so that hte coordinates range from 0 to 1
...
{
	// Compute the bounding box center y-coordinates.
	return (y1_ + y2_) / 2;
}
						  ...
						  BoundingBox* bbox_uncentered) const
{
	...
	bbox_uncentered->x2_ = std::min(static_cast<double>(raw_image.cols), 
									x2_ + search_location.x1_ - edge_spacing_x);
	bbox_uncentered->y2_ = std::min(static_cast<double>(raw_image.rows), 
									y2_ + search_location.y1_ - edge_spacing_y);
}
...
double BoundingBox::edge_spacing_x() const
{
	...
	// Compute the amount that output "sticks out" beyond the edge of the image 
	// (edge effects).
	// If there are no edge effects, we would have output_width / 2 < bbox_center_x, 
    // but if the crop is near the left
    // edge of the image then we would have output_width / 2 > bbox_center_x, 
    // with the difference
...
{
	// Draw a white dounding box on the image.
	Draw(225, 225, 225, image);
}
/****************************************************************************************
** @function:
** @return: void
** @params:
**   image: current train frame
**   lambda_scale_frac, lambda_shift_frac, min_scale, max_scale: default Bounding 
**     box params
**   shift_motion_model:  Choose whether to shift boxes using the motion model 
**     or using a uniform distribution.
**   bbox_rand: The 10 shift result
***************************************************************************************
**/
					    ...
					    BoundingBox* bbox_rand) const 
{
    ...
    // Sample a width scaling factor for the new crop window, thresholding the scale to
	// stay within a reasonable window.
	double new_width = -1;
    ...
    while ((new_width < 0 || new_width > image.cols - 1) && num_tries_width < kMaxNumTries) {
		...
		if (shift_motion_model) {
			...
			width_scale_fractor = rand_num * (max_scale - min_scale) + min_scale;
		}
	...
	// Find a height scaling factor for the new crop window, thresholding the scale to 
	// stay within a reasonable window.
	double new_height = -1;
	...
	while ((first_time_x || 
		...
		double new_x_temp;
		if (shift_motion_model) {
			new_x_temp = center_x + width * sampel_exp_two_sided(lambda_shift_frac);
		} else {
			const double random_num = sample_rand_uniform();
	...
	while ((first_time_y || 
		   // Ensure that the new object center remains in the old image window. 
		   new_center_y < center_y - width * kContextFactor / 2 ||
		   new_center_y > center_y + width * kContextFactor / 2 ||
		   // Ensure that the new window stays within the borders of the image.
			new_center_y - new_width / 2 < 0 ||
			new_center_y + new_width / 2 > image.cols) 
		  && num_tries_y < kMaxNumTries) {
		// Sample.
		double new_y_temp;
		if (shift_motion_model) {
			new_y_temp = center_y + width * sampel_exp_two_sided(lambda_shift_frac);
		} else {
			const double random_num = sample_rand_uniform();
			new_y_temp = center_y + rand_num * (2 * new_width) - new_width;
		}
	    // Make sure that the window stays within the image
		new_center_y = min(image.cols - new_width / 2, max(new_width / 2, new_x_temp));
		first_time_y = false;
		num_tries_y++;
	}
...
{
	const double area = std::max(0.0, std::min(x2_, bbox.x2_) - std::max(x1_, bbox.x1_)) *
		                std::max(0.0, std::min(y2_, bbox.y2_) - std::max(y1_, bbox.y1_));
	return area;
}

double BoundingBox::coumpute_are() const
{
