...
/*********************************************************************************
** @function:  compute the bounding box in the t frame
** @return: NULL
** @params:
**   bbox_tight: the t-1 frame boungding box location
**   image: the t frame
**   pad_image_location: the bounging box of t frame 
**********************************************************************************
*/
								 ...
								 BoundingBox* pad_image_location)
{
	...
	// The output image is centered on the bounding box center but has a size given by 
	...
	// Compute the output image size, limiting the output to within the borders of 
	// the original image
	
	// The left half of the output has a width of output_width / 2, unless it is bounded by 
	// the left image border,
	// in which case it has a width of bbox_center_x.
	const double left_half = std::min(output_width / 2, bbox_center_x);
	
	// The right half of the output has a width of output_width / 2, unless it is bounded 
	// by the right image border,
	...
	// The total width of the output is the sum of the widths of the two halves, 
	// but cannot be smaller than 1 pixel.
	const double roi_width = std::max(1.0, left_half + right_half);
	
	// The top half of the output has a height of output_height / 2, 
	// unless it is bounded by the top image border.
	const double top_half = std::min(output_height / 2, bbox_center_y);
	
	// The bottom half of the output has a height of output_height / 2, 
	// unless it is bounded by the bottom image border
	const double bottom_half = std::min(output_height / 2, image_height - bbox_center_y);
	
	// The total height of the output is the sum of the heights of the two halves, 
	// but cannot be smaller than 1 pixel
...
/*************************************************************************************
** @function : compute the region of object in t frame
** @return : NULL
** @params:
**   bbox_tight: the boundingbox location in the t-1 frame
**   image: the t frame
**   pad_image: the region of object in t frame
***************************************************************************************
*/
...
/***************************************************************************************
** @function: compute the region and the location of the object in t frame
** @return : NULL
** @params :
**   bbox_tight: the bounding box location in t-1 frame
**   image: the t frame
**   pad_image: the result of object in t frame
**   pad_image_location: teh location of object in t frame
**   edge_spacing_x: 
**   edge_spacing_y:  
*****************************************************************************************
*/
                 ...
                 BoundingBox* pad_image_location, double* edge_spacing_x, double* edge_spacing_y)
{
	...
	const double roi_bottom = std::min(pad_image_location->y1_,
	...
	const double roi_height = std::min(static_cast<double>(image.rows), std::max(1.0, ceil(pad_image_location->y2_ - pad_image_loaction->y1_)));
	...
	*edge_spacing_x = std::min(bbox_tight.edge_spacing_x(), 
							   static_cast<double>(output_image.cols - 1));
	*edge_spacing_y = std::min(bbox_tight.edge_spacing_y(), 
							   static_cast<double>(output_image.rows - 1));
	
	// Get the location within the output to put the cropped image 
