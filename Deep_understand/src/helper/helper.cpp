...
// Maximum length of a string created form a number.
...
void find_subfolders(const bfs::path& folder, vector<string>* sub_folders)
{
	...
	bfs::directory_iterator end_itr; // default construction yileds past-the-end
						...
						vector<string>* files)
{
	...
	for (bfs::directory_iterator itr(folder); itr != end_itr; ++itr) {
		if (bfs::is_regular_file(itr_status())) {
			string filename = itr->path().filename().string();
...
double sample_exp(const double lambda)
{
	...
	// http://stackoverflow.com/questions/11491458/how-to-generate-random-numbers-with-exponential-distribution-with-mean
