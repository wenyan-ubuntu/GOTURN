...
#include "helper/high_re_timer.h"
...
void VideoLoader::ShowVideosShift() const
{
	...
	for (size_t video_index = 0; video_index < videos_.size(); ++video_index) {
		...
		const std::vector<Frame>& annotations = video.annotations_;
		
		BoundingBox bbox_prev;
		cv::Mat image_prev;
		
		// Create an example generator with parameters for a large shift, so that 
		// this will be noticable.
		// This will be used to artifically shift the crops around the annotations, 
		// creating an
		...
		for (size_t frame_index = 0; frame_index < annotations.size(); ++frame_index) {
			...
			// If this is the second (or later) bounding box, show the previous target and
			// the current
			// search region (based on the object's previous location), with corresponding 
			// annotations.
			if (frame_index > 0) {
...
#ifndef NO_DISPLAY
				cv::nameWindow("Raw image", cv::WINDOW_AUTOSIZE);
				cv::imshow("Raw image", full_image_with_bbox);
#endif
				...
				for (int k = 0; k < kNumGeneratedExamples; ++k) {
					// Shift the cropped region to generate an apparent motion 
					// (translation and scale chaneg) and visualize.
