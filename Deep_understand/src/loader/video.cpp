...
void Video::ShowVideo() const 
{
	...
	// Iterator over all frames in this video.
	for (size_t image_frame_num = start_frame; image_frame_num <= end_frame; ++image_frame_num) {
		...
		if (annotated_frame_num == image_frame_num) {
			...
			const BoundingBox& box = annotations_[annotated_frame_index].bbox;
		...
		if (!image.data) {
			printf("Coulde not open or dind image %s\n", image_file.c_str());
		} else {
#ifndef NO_DISPLAY
			cv::nameWindow("Display window", cv::WINDOW_AUTOSIZE); // Create a window for display
			cv::imshow("Display window", image); // Show our image inside
...
{ 
	...
	const Frame& annotated_frame = annotations_[annotation_index];
	...
	*box = annotated_frame.bbox;
	...
	if (image_files.empty()) {
		printf("Error - np image files for video at path: %s\n", path.c_str());
		return;
...
bool Video::FindAnnotation(const int frame_num, BoundingBox* box) const
{
	...
	for (size_t i = 0; i < annotations_.size(); i++) {
		const Frame& frame = annotations_[i];
					  ...
					  BoundingBox* box) const 
{
	...
	// Draw the annotation (if it exits) on the image.
