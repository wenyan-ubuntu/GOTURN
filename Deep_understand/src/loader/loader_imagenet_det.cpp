...
// If the ratio is to large (i.e. the object occupies alomost the entire image),
// then we will not beable to simulate objection motion.
const double kMaxRation = 0.66;
	...
	: path_(image_folder)
{
	...
	for (size_t i = 0; i < max_subfolders; i++) {
		// Every 100 iterators, print an update.
		if (i % 10 == 0 && i > 0) {
		...
		printf("Loading subfolder: %s\n", subfolder_name.c_str()); // add by wendian-wei
		printf("Loading subfolder path: %s\n", subfolder_path.c_str()); // add by wendian-wei
		...
		// Iterator over all annotation files.
...
** @return: void
** @params:
**   annotation_file: the path of subfolder
**   image_annotations: all the annotations in the subfolder
****************************************************************************************
*/
...
void LoaderImagenetDet::ComputeStatistics() const
{
	...
	// Variables to stroe vairous image statistics.
	double min_width;
	double min_height;
	double max_width;
	...
	for (size_t i = 0; i < images_.size(); i++) {
		...
		for (sizr_t j =0; j < annotations.size(); j++) {
			...
			if (first_time) {
				...
				// fraction of the image width / height occupied by the boudnign box.
				...
				max_widht_frac = std::max(max_width_frac, width_frac);
									  ...
									  BoundingBox* bbox) const
{
	...
	*image = cv::imread(image_file.c_str());
	...
	// Check if the display width / height differs from the image width / height 
	// (the image may have been downsampled for visualization). Usually this value will be 1
	double factor = 1;
	if (image->rows != annotation.display_height_ || image->cols != annotation.display_width_) {
		...
		// Check that the aspect ratio was presevered for annotation.
...
{
    // Create an example generator with parameters for a large shift, 
    // so that this will be noticable.
    // This will be used to artificially shift the crops around the annotations, 
    // creating an
	...
	for (size_t i = 0; i < images_.size(); i++) {
		...
		for (size_t j = 0; j < annotations.size(); j++) {
			...
			// Make a training example from randomly shifting the annotation and 
			// visualize it.
