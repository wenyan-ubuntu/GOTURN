...
public:
	...
	void LoadFirstAnnotation(int* first_frame, cv::Mat* image, BoundingBox* box) const;
	
	// For a given frame num, find an annotation if it exists, and return ture.
	// Otherwise return flase.
	// If load_only_annotation = false, alse load the image.
	...
	// Note that the length of this vector may different from the length of the
	// vector of the image filenames,
	// if only a strict subset of video frames were labeled.
	std::vector<Frame> annotations_;
