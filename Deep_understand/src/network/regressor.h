...
public:
	...
	// image is the best guess as to a crop of the current image that likely conntains 
	// the target object.
	// target is an image of the target object from the previous frame.
	// Return: bbox, an estimated location of the target object in the current image.
	virtual void Regress(const cv::Mat& image_curr, const cv::Mat& image, 
	                     const cv::Mat& target, BoundingBox* bbox);
...
protected:
	...
	// Reshape the image inputs to the network to match the expected size and number of
	// images.
	virtual void ReshapeImageInputs(const size_t num_images);
	
	// Get the features in the network with the given name, and copy their values to 
	// the output.
	void GetFeatures(const std::string& feature_name, std::vector<float>* output) const;
	
	// Pass the image and the target to the network;
	// estimate the location of the target in the current image.
	...
	// Wrap the input layer of the network in sparate cv::Mat objects
	// (one per channel)
	...
	// If the parameters of the network have been modified, reinitialize 
	// the parameters to their original values.
	virtual void Init();
	
private:
    // Set up a network with architecture specified in deploy_proto,
	// With the model weights saved in caffe_model.
...
private:
    // Number of inputs expected by hte network.
	int num_inputs_;
