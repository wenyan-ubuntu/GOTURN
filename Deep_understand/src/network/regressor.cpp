...
void Regressor::Estimate(const cv::Mat& image, const cv::Mat& target, 
						 std::vector<float>* output)
{
	...
	// Forwaed dimension change to all layers.
	net_->Reshape();
...
void Regressor::GetFeatures(const string& feature_name, std::vector<float>* output) const
{
	...
	// Compute the number of element in this layer.
	int num_elements = 1;
	...
	*output = std::vector<float>(begin, end);
	...
	for (int i = 0; i < *output.size(); i++) {
		printf("The %d-th elements in output vector %f", i, (*output)[i]);
	}
}
						  ...
						  const std::vector<cv::Mat>& targets)
{
	...
	// Set network inputs to the apportinate size and number.
...
// layer.	
void Regressor::WrapInputLayer(std::vector<cv::Mat>* target_channels,
							   std::vector<cv::Mat>* image_channels) 
{	
