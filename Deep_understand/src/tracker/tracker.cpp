...
#include <opencv2/videostab/onpainting.hpp>

#include "helper/helper.h"
#include "helper/bounding_box.h"
#include "network/regressor.h"
#include "network/regressor_train.h"
#include "network/high_res_timer.h"
...
void Tracker::Track(const cv::Mat& image_curr, RegressorBase* regressor,
	...
	// Estimate the bounding box location of the target, centered and scaled relative to 
    // to the cropped image.
	BoundingBox bbox_estimate;
	regressor->Regress(image_curr, curr_search_region, target_pad, &bbox_estimate);
	
	// Unscale the estimate to the real image size
	BoudningBox bbox_estimate_unscaled;
	...
	bbox_estimate_unscaled.Uncenter(image_curr, search_location, 
									edge_spacing_x, edge_spacing_y,
								    bbox_estimate_uncentered);
	...
	// Save the currrent estimate as the location of the target.
	bbox_prev_tight_ = *bbox_estimate_uncentered;
	
	// Save the currrent estimate as the prior prediction for the next image.
	// TODO - replace with a motion model prediction?
	bbox_curr_prior_tight_ = *bbox_estimate_uncentered;
}

void Tracker::ShowTracking(const cv::Mat& target_pad, const cv::Mat& curr_search_region,
						   const BoundingBox& bbox_estimate) const 
{
	// Resize the target.
