...
public: 
	...
	// Initialize teh tracker with the ground-truth bounding box of the first frame.
	...
	// VOTRegion is an object for initializing the tracker when using the VOT Tracking 
	// dataset.
	void Init(const std::string& image_curr_path, const VOTRegion& region,
              RegressorBase* regressor);

private:
    // Show the tracking output, fro debugging.
	void ShowTracking(const cv::Mat& target_pad, const cv::Mat& curr_search_region, 
