...
{
	// Iterator over all videos and track the target object in each.
	for (size_t video_num = start_video_num; video_num < videos_.size(); ++video_num) {
		...
		// Get the first frame of this video with the initial ground-truth bounding box
		...
		// Iterate over the remain frames of the video.
		printf("Frames: ");
		size_t frame_num = first_frame + 1;
		for (; frame_num < video.all_frames_.size(); ++frame_num) {
			if (frame_num % 100 == 0) {
			...
			// (The fround-truth bounding box is used only for visualization).
			...
			// Get ready top track the object.
			SetupEstimate();
...
TrackerVisualizer::TrackerVisualizer(const std::vector<Video>& videos,
									 RegressorBase* regressor, Tracker* tracker)
	: TrackerManager(videos, regressorm tracker)
{
}
