...
public:
	...
	// Set up to train on the previous and current image, and the previous and 
	// current bounding boxes.
	void Reset(const BoundingbBox& bbox_prev, const BoundingBox& bbox_curr,
	...
	void set_indies(const int video_index, const int frame_index) {
...
private:
	...
	void VisualizeExample(const bool save_images = false,
	...
	// To generate synethic examples, shift the bounding box by an exponential with 
	...
	// Do not scale the synthetic examples by more than max_scale_ shrink them 
	// by more than min_scale_.
	double min_scale_;
	double max_scale_;
