...
public:
	...
	// Inputs: previous image, current image, previous image's bounding box, 
	// current image's bounding box.
	void Train(const cv::Mat& image_prev, const cv::Mat& image_curr,
	           const BoundingBox& bbox_prev, const BoundingBox& bbox_curr);
			   
	// Number of tatal batches trained on so far.
...
private:
	...
	// some examples then then we will append to them
