												  ...
												  BoundingBox* bbox_gt_scaled) const
{
	*target_pad = target_pad_;
	...
	// Crop the image based at the new location 
	// (after applying translation and scale changes).
	double edge_spacing_x, edge_spacing_y;
	...
	// Find the shifted ground-truth bounding box loaction relative to the image crop
	BoundingBox bbox_gt_recentered;
...
** @params
**/
void ExampleGenerator::VisualizeExample(const bool save_images,
	                                    const cv::Mat& target_pad,
									    ...
									    const BoundingBox& bbox_gt_scaled) const
{
	...
	if (save_images) {
		const string target_name = "Image" + num2str(video_index_) + "_" + 
			num2str(frame_index_) + "target.jpg";
...
#ifndef NO_DISPLAY
	cv::nameWindow("Search_region+gt", cv::WINDOW_AUTOSIZE); // Create a window for display.
	cv::imshow("Search_region+gt", image_resize); // Show our image inside it.
	cv::waitKey(0);
#endif
	
	if (save_images) {
		const string image_name = "Image" + num2str(video_index_) + "_" +
			num2str(frame_index_) + "image.jpg";
